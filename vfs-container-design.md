...
The rename operation has specific semantics to maintain filesystem invariants:

```
RENAME(source, dest):
  
  1. CYCLE PREVENTION (directories only):
     - If source is a directory, check that dest's parent is not 
       a descendant of source
     - If dest_parent is in source's subtree: return WouldCreateCycle
  
  2. OVERWRITE RULES:
     - File → File: overwrite (delete dest, move source)
     - Dir → Empty Dir: overwrite (delete dest, move source)
     - Dir → Non-empty Dir: ERROR (DirectoryNotEmpty)
     - File → Dir: ERROR (NotAFile)
     - Dir → File: ERROR (NotADirectory)
     - Anything → Non-existent: create (move source to dest)
  
  3. ATOMICITY:
     - Rename is atomic within a transaction
...
fn check_rename_cycle(
    ...
    // Walk from dest_parent up to root
...
```
REMOVE(path):
  - File: delete node and decrement content reference
  - Empty Directory: delete node
  - Non-empty Directory: ERROR (DirectoryNotEmpty)
  - Symlink: delete symlink node (not target)

REMOVE_ALL(path):
  - Recursively delete all descendants
  - Then delete the node itself
  - Content cleanup via reference counting
```

#### 6.5.3 Hard Link Rules (when enabled)

```
HARD_LINK(link_path, target_path):
  - Target must be a FILE (not directory, not symlink)
  - Creates new edge pointing to same NodeId
  - Increments link_count on node
  - Both paths now refer to same content

DELETE with hard links:
  - Decrement link_count
  - Remove edge only
  - Content deleted when link_count reaches 0
```
...
### 9.4 Quota Accounting Rules

#### 9.4.1 Basic Accounting

```
QUOTA ACCOUNTING (Logical Bytes Model):

1. total_size = SUM of (size field) for all file nodes
   - Each file node has its own size field
   - This is "logical size" (what user perceives), not physical storage

2. node_count = COUNT of all nodes (files + dirs + symlinks)
```

#### 9.4.2 Hard Link Behavior

When hard links are enabled, multiple directory entries can point to the same content:

```
HARD LINK ACCOUNTING:

1. Creating hard link:
   - New node created with same content_id as target
   - New node has its own size field (equal to target's size)
   - total_size INCREASES by the new node's size
   - node_count INCREASES by 1

2. Rationale:
   - Quotas should reflect "apparent" usage
   - User sees 2 files of 1MB each = 2MB used
   - Predictable behavior (no "I deleted a file but quota didn't change")

3. Delete with hard links:
   - Deleting a node: total_size DECREASES by node's size
   - Deleting a node: node_count DECREASES by 1
   - Content (chunks) deleted when NO nodes reference content_id
   - Reference counting via link_count in content metadata
```
...
1. Copy creates new node with NEW content_id
2. Content data is duplicated (no implicit sharing)
3. total_size INCREASES by copied file's size
4. node_count INCREASES appropriately

Note: Copy does NOT preserve hard links.
      If source has hard links, copy creates independent file.
```

#### 9.4.4 Content Garbage Collection

```
CONTENT GC:

1. Each content_id has a reference count (stored in metadata)
2. When node deleted: decrement refcount for its content_id
3. When refcount reaches 0: delete all chunks for content_id
4. GC happens within the same transaction as delete

No background GC needed - cleanup is synchronous.
```
...
pub enum VfsError {
    ...
    #[error("rename would create directory cycle: {source} -> {dest}")]
...
2. For multi-threaded use, wrap externally:
   - Arc<Mutex<FilesContainer<B>>> for shared mutable access
   - Arc<RwLock<FilesContainer<B>>> for read-heavy workloads
   - Or use connection pool pattern (one container per thread)

3. Backend-level concurrency:
   - SQLite has internal locking (safe for concurrent readers)
...
let container = Arc::new(Mutex::new(FilesContainer::new(backend)));
...
```rust
// Each thread gets its own connection to the same SQLite file
thread_local! {
    static CONTAINER: RefCell<FilesContainer<SqliteBackend>> = {
        RefCell::new(FilesContainer::new(
            SqliteBackend::open("data.db").unwrap()
        ))
    };
}
```
...
> **Note:** `vfs-fs` (host filesystem backend) is **deferred from MVP**. See ADR and review response for rationale. The graph-store trait requires persistent NodeId identity, which doesn't map cleanly to a host filesystem without a sidecar index.
...
| Solution | Focus | Backend Model | Maturity |
|----------|-------|---------------|----------|
| **vfs** (1.5M downloads) | General VFS abstraction | Path-based operations | Production |
| **OpenDAL** (Apache) | Cloud/object storage | Object storage ops | Production |
| **virtual-filesystem** | std-conformant VFS | Path-based operations | Stable |
| **VFS Container** (ours) | Isolated data containers | Direct or Node/Edge ops | Design phase |

### 14.2 Key Differentiators

**What VFS Container adds:**

| Feature | vfs | OpenDAL | VFS Container |
|---------|-----|---------|---------------|
| Validated path types | ❌ `&str` | ❌ `&str` | ✅ `VirtualPath` |
| Built-in capacity limits | ❌ | ❌ | ✅ |
| Feature toggles (symlinks, etc.) | ❌ | N/A | ✅ |
| SQLite-first design | ❌ | ❌ | ✅ |
| Mandatory transactions | ❌ | ❌ | ✅ (node/edge) |
| Path traversal safety in core | ⚠️ Backend | ⚠️ Backend | ✅ Core |

**What alternatives do better:**

| Feature | vfs | OpenDAL | VFS Container |
|---------|-----|---------|---------------|
| Backend variety | ✅ 5 built-in | ✅ 50+ | ⚠️ 3 planned |
| Streaming I/O | ✅ | ✅ | ❌ Bulk only |
| Async support | ✅ | ✅ Primary | ❌ Sync only |
| Overlay/composition | ✅ OverlayFS | ❌ | ❌ |
| Production maturity | ✅ Years | ✅ Years | ❌ New |

### 14.3 Use Case Alignment

**Use VFS Container when:**
- Multi-tenant SaaS with per-tenant quotas
- Portable single-file data containers
- Security-critical sandboxing
- Offline-capable desktop applications
- Testing with deterministic filesystem behavior

**Use vfs when:**
- Need overlay/layered filesystems
- Embedding resources in executable
- Drop-in filesystem abstraction for tests
- Already integrated in your project

**Use OpenDAL when:**
- Cloud storage (S3, GCS, Azure, HDFS)
...
**Our niche:** Portable, isolated, quota-controlled data containers backed by SQLite.

**Not competing with:** Cloud storage (OpenDAL) or container-style layering (vfs OverlayFS).

**Risk:** As a new project, we lack production battle-testing that vfs and OpenDAL have.

**Mitigation:** Start with filesystem backend via `strict-path` to validate abstractions before investing in SQLite complexity.

*For detailed technical comparison, see [Technical Comparison with Alternatives](./08-technical-comparison-with-alternatives.md).*
