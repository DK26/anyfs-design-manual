...
├──────────┬──────────┬───────────────────┤
│ VRootFsBackend│MemBackend│  SqliteBackend    │
└──────────┴──────────┴───────────────────┘
```
...
└── src/
    ...
    ├── error.rs        # VfsError
    │
    ├── vrootfs/        # Filesystem backend
    │   └── mod.rs      # VRootFsBackend (via strict-path)
    │
...
### VRootFsBackend (via strict-path)
...
```rust
use anyfs::{VfsBackend, VRootFsBackend, MemoryBackend, SqliteBackend, VirtualPath};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    // Filesystem backend
    let mut fs = VRootFsBackend::new(Path::new("/tmp/myapp"))?;
...
```rust
use anyfs::{VRootFsBackend, SqliteBackend, VirtualPath};
...
2. `VfsBackend` trait
3. `VRootFsBackend` via `strict-path`
4. `MemoryBackend` 
5. Tests
